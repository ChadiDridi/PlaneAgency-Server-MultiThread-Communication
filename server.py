...
def NotificationServeur(ip,mess,csock):
    ...
    if elements[0] == "Reservation":
        serverMutex.acquire()
        if  Resrver(int(elements[1]),int(elements[2]),int(elements[3])) :  
    ...
    if elements[0] == "Annulation":
        serverMutex.acquire()
        if Annuler(int(elements[1]),int(elements[2]),int(elements[3])):
...
def Annuler(ref,nb,refAgence):
    if(verificationVol(ref)):
        ...
        for j in range(len(lnhisto)):
             clhisto=lnhisto[i].split(',')
             if int(ref)==int(clhisto[0]) & int(refAgence)==int(clhisto[1]) & clhisto[5]=="succes" :
               if  clhisto[2]=="Demande" :
        ...
        if compteur>=nb :  
              ...
              for i in range(len(lnfact)):
                ...
                if int(fl[0])==int(refAgence):
                   fl[1]=int(fl[1])-(0.1*nb*prix)        
                   lnfact[i]="{},{}".format(refAgence,fl[1]) 
